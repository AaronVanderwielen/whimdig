function initSchema(m) {
	...
	var eventSchema = new m.Schema({
		...
		_messages: [{ type: m.Schema.Types.ObjectId, ref: 'Message' }],
	});
	...
	var messageSchema = new m.Schema({
		date: Date,
		text: String,
		_user: { type: m.Schema.Types.ObjectId, ref: 'User' },
		_event: { type: m.Schema.Types.ObjectId, ref: 'Event' }
	})

	var Event = m.model('Event', eventSchema);
	Event.schema.path('_tags').validate(function (value) {
	...
	var Message = m.model('Message', messageSchema);
