			...
			actions: {
				get: {
					...
					span: {
						...
						method: function (req, res) {
							// get events from mongodb
							bl.getAllEventsForSpan(req.query.span, function (data) {
								res.json(new jsonResponse(true, data));
							});
						}
					},
				...
				post: {
					...
					addUser: {
						...
						method: function (req, res) {
							if (req.body.eventId) {
								bl.addUserToEvent(req.session.user._id, req.body.eventId, function (event) {
					...
					removeUser: {
						...
						method: function (req, res) {
							if (req.body.eventId) {
								bl.removeUserFromEvent(req.session.user._id, req.body.eventId, function (event) {
									if (event) {
										res.json(new jsonResponse(true, event));
									}
									else {
	...
	// account
		{
			...
			actions: {
				...
				post: {
					authenticate: {
						method: function (req, res) {
							bl.authenticate(req.body.token, req.body.id, function (user) {
								if (user) {
									console.log("setting session to:");
									console.log(JSON.stringify(user));
									req.session.user = user;
									res.json(new jsonResponse(true, user));
								}
								else {
