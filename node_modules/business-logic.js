var BusinessLogic = function (opts) {
	...
	// event
    this.getUpcomingEvents = function (tags, span, radius, loc, fbId, callback) {
    	if (tags) {
    		for (var f in tags) {
    ...
    this.getEventsForBounds = function (span, filters, bounds, fbId, callback) {
    	var now = new Date(),
			hours = 12,
		...
		s.place.findOne(query, function (err, doc) {
			syncHandler(err, doc, function (response) {
				...
				if (eventData.privacy === "private") {
				    eventData.invited = user.friends;
				...
				eventData.tags = eventData.tags.split(",").filter(function (e) { return e });
	...
	this.updateEvent = function (eventData, user, callback) {
	    var response = new jsonResponse(true, null),
            eventQuery = { _id: new ObjectID(eventData._id), created_by: new ObjectID(user._id) },
	    ...
	    s.event.findOne(eventQuery, function (err, doc) {
	        syncHandler(err, doc, function (response) {
	            ...
	            dbEvent.tags = eventData.tags;

	            // set privacy
	            dbEvent.privacy = eventData.privacy;
	            if (dbEvent.privacy === "private") {
	                dbEvent.invited = user.friends;
	                dbEvent.invited.push(user.facebook_id);
	            }

	            // set place
	            dbEvent.place = new ObjectID(eventData.place);
					...
					if (!err) {
						bl.getFacebookFriends(token, function (friends) {
							...
							if (dbUser === null) {
							    ...
							    if (fbUser.facebook_id === '10204770365593869') {
							        fbUser["admin"] = true;
							    }
	...
	this.getPlaceUpcomingEvents = function (placeId, fbId, callback) {
	    var now = new Date(),
			query = {
			    end: { $gt: now },
			    $or: [{ privacy: 'public' }, { privacy: 'private', invited: fbId }],
