var BusinessLogic = function (Mongo, Https, underscore) {
	var bl = this,
		db = Mongo.db,
	...
	this.getAllEvents = function (callback) {
		var now = new Date(),
			...
			query = { start: { $lt: maxHigh }, end: { $gt: now }, privacy: 'public' };
	...
	this.getAllEventsForSpan = function (span, callback) {
		var now = new Date(),
			hours = span == 0 ? 12 : (span == 1 ? 24 : 48),
			high = new Date(now.getTime() + (3600000 * hours)),
			query = { start: { $lt: high }, end: { $gt: now }, privacy: 'public' };
	...
	this.checkFacebookToken = function (token, userID, callback) {
	...
	this.authenticate = function (token, userID, callback) {
		// verify with facebook
		bl.checkFacebookToken(token, userID, function (auth) {
			if (auth.facebook_id) {
				// query database for user
				var query = { facebook_id: auth.facebook_id };
				db.collection('users').findOne(query, function (err, doc) {
					if (!err) {
						if (doc === null) {
							// create user in our db
							console.log("create user in our db");
							db.collection('users').insert(auth, function (err, doc) {
								callback(doc);
							});
						}
						else {
				...
				bl.placesApiSearchByName(key, lat, lng, name, function (response) {
					if (response.success) {
						...
						for (var p in response.body) {
							...
							if (!u.some(placesArray, function (pl) { return pl.place_id === response.body[p].place_id; })) {
