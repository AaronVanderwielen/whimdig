var BusinessLogic = function (opts) {
	var bl = this,
		...
		cloneEventForWeek = function (e, weeks) {
			var clone = JSON.parse(JSON.stringify(e));
			clone.start = moment(clone.start).add(weeks, 'w');
			clone.end = moment(clone.end).add(weeks, 'w');
			return clone;
		};
	...
	this.getEventsForBounds = function (span, filters, bounds, userId, callback) {
		span = span > 48 ? 48 : (span < 12 ? 12 : span);
		var now = new Date(),
			hours = span,
	...
	this.authenticate = function (token, fbUserID, callback) {
		...
		bl.checkFacebookToken(token, fbUserID, function (fbUser) {
			if (fbUser.facebook_id) {
				...
				s.user.findOne(userQuery)
					.populate('_mail', 'read_log')
				...
				callback(new jsonResponse(false, "Facebook token authentication failed"));
			}
		});
	};
	//// mail
	...
	this.createEventAdmin = function (eventData, callback) {
		...
		// get place
		s.place.findOne(query, function (err, place) {
			if (!err && place) {
				console.log('creating event: ' + eventData.name);
				
				eventData.loc = place.loc;

				// convert string Ids to ObjectIDs
				var tagIds = eventData._tags.split ? eventData._tags.split(",") : eventData._tags;
				for (var t in tagIds) {
					tagIds[t] = new ObjectID(tagIds[t]);
				}
				eventData._tags = tagIds;
				...
				console.log('could not find place: ' + eventData.name);
				callback(new jsonResponse(false, 'could not find place'));
			}
		});
	};
	...
	this.importEventsCsv = function (data, callback) {
		...
		for (var d in data) {
			var eventData = {
				_place: data[d][0].trim(),
				name: data[d][2].trim().replace(/[^A-Za-z 0-9 \.,\?""!@#\$%\^&\*\(\)-_=\+;:<>\/\\\|\}\{\[\]`~]*/g, ''),
				desc: data[d][3].trim().replace(/[^A-Za-z 0-9 \.,\?""!@#\$%\^&\*\(\)-_=\+;:<>\/\\\|\}\{\[\]`~]*/g, ''), // strip non ascii characters
				start: moment(data[d][4], "M-D-YYYY h:mma"),
				end: moment(data[d][5], "M-D-YYYY h:mma"),
				intensity_variable: data[d][6].trim(),
				privacy: 'public',
				messages: [],
				_tags: data[d][7].trim(),
			};

			if (eventData.desc.length > 1000) {
				eventData.desc = eventData.desc.substring(0, 997) + "...";
			}
			if (eventData.name.length > 60) {
				eventData.name = eventData.name.substring(0, 57) + "...";
			}

			eventQueue.push(eventData);

			if (data[d][8]) {
				var repeated = data[d][8].trim();
				if (repeated === 'weekly') {
					eventQueue.push(cloneEventForWeek(eventData, 1));
					eventQueue.push(cloneEventForWeek(eventData, 2));
					eventQueue.push(cloneEventForWeek(eventData, 3));
				}
				else if (repeated === 'bi-monthly') {
					eventQueue.push(cloneEventForWeek(eventData, 2));
				}
			}
		}
		...
		for (var e in eventQueue) {
			bl.createEventAdmin(eventQueue[e], function (response) {
				if (response.success) console.log(response.body.name + " imported");
				else console.log("failed");

				numUploaded++;
				if (numUploaded == eventQueue.length) {
					callback();
				}
			});
		}
	...
	this.createDefaultTags = function (callback) {
		var filters = [
			{
				glyph: "glyphicons-music",
				label: "Music"
			},
			{
				glyph: "glyphicons-celebration",
				label: "Alcohol"
			},
			{
				glyph: "glyphicons-fast-food",
				label: "Food"
			},
			{
				glyph: "glyphicons-cardio",
				label: "Move"
			},
			{
				glyph: "glyphicons-playing-dices",
				label: "Games"
			},
			{
				glyph: "glyphicons-turtle",
				label: "Funny"
			},
			{
				glyph: "glyphicons-gamepad",
				label: "Gaming"
			},
			{
				glyph: "glyphicons-book",
				label: "Books"
			},
			{
				glyph: "glyphicons-child",
				label: "Kids"
			},
			{
				glyph: "glyphicons-dog",
				label: "Pets"
			},
			{
				glyph: "glyphicons-magic",
				label: "Theatre"
			},
			{
				glyph: "glyphicons-rugby",
				label: "Sports"
			},
			{
				glyph: "glyphicons-bank",
				label: "Learn"
			},
			{
				glyph: "glyphicons-brush",
				label: "Art"
			},
			{
				glyph: "glyphicons-wifi",
				label: "Tech"
			},
			{
				glyph: "glyphicons-film",
				label: "Film"
			},
			{
				glyph: "glyphicons-group",
				label: "Community"
			},
			{
				glyph: "glyphicons-tags",
				label: "Garage Sale"
			},
			{
				glyph: "glyphicons-home",
				label: "Open House"
			},
			{
				glyph: "glyphicons-birthday-cake",
				label: "Birthday"
			}
		]
