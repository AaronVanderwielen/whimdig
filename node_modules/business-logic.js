	...
	this.importEventsCsv = function (data, user, callback) {
		...
		for (var d in data) {
			var eventData = {
				...
				intensity_variable: data[d][5].trim(),
			};

			var repeated = data[d][6].trim(),
				iterations = 1;
			...
			for (var i = 0; i < iterations; i++) {
				...
				// probably need moment. setDate can't add days and go to new month
			}
		}
		for (var i = 0; i < events.length; i++) {
			bl.createEventAdmin(events[i], function (response) {
				if (response.success) console.log(response.body.name + " imported");
				else console.log("failed");

				numUploaded++;
				if (events.length == numUploaded) {
					callback();
				}
			});
		}
