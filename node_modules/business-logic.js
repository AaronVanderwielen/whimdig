var BusinessLogic = function (opts) {
	...
	this.getAllEvents = function (fbId, callback) {
		...
		db.collection('events').find(query).toArray(function (err, doc) {
			syncHandler(err, doc, callback, true);
		});
	};
	...
	this.getUser = function (userId, callback) {
		...
		s.user.findOne(query, function (err, user) {
			console.log("GOT USER WITH MONGOOOSE!!! " + user._id);
			syncHandler(err, doc, callback);
		})
	...
	this.authenticate = function (token, fbUserID, callback) {
		...
		bl.checkFacebookToken(token, fbUserID, function (fbUser) {
			if (fbUser.facebook_id) {
				...
				s.user.findOne(query, function (err, dbUser) {//db.collection('users').findOne(query, function (err, dbUser) {
					console.log("GOT USER WITH MONGOOOSE!!! " + dbUser._id);
					if (!err) {
						bl.getFacebookFriends(token, function (friends) {
							fbUser.friends = _.map(friends, function (f) { return f.facebook_id; });
	...
	this.getPlaceById = function (id, callback) {
		...
		db.collection('places').findOne(query, function (err, doc) {
			syncHandler(err, doc, callback);
		});
	};
	this.getPlacePastEvents = function (placeId, fbId, callback) {
		var now = new Date(),
			query = {
				...
				$or: [{ privacy: 'public' }, { privacy: 'private', invited: fbId }],
				'place._id': new ObjectID(placeId)
			},
			options = {
				limit: 15,
				sort: { end: -1 }
		...
		db.collection('events').find(query, options).toArray(function (err, doc) {
			syncHandler(err, doc, callback);
		});
	};
