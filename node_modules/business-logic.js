var BusinessLogic = function (opts) {
	...
	//// event messaging
	this.addMsgToEvent = function (msg, eventId, userId, callback) {
		var messageData = {
			text: msg,
			date: new Date(),
			_user: userId,
		...
		message = new s.message(messageData);

		message.save(function (err, doc) {
			if (!err) {
				var query = { _id: new ObjectID(eventId), _users: userId },
					update = { $push: { _messages: doc._id } };
				...
				s.user.findOne(userQuery, function (err, dbUser) {
					if (!err) {
						bl.getFacebookFriends(token, function (friends) {
							...
							bl.getUserIdsByFbIds(fbFriendIds, function (response) {
								var userIds = response.body;
								if (dbUser === null) {
